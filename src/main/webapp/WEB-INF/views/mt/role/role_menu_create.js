				...
				$(this).change(function(){					
					...
					$($("select").get(2)).each(function(){
						$(this).change(function(){
							...
							$($("select").get(3)).each(function(){
								$(this).change(function(){
									$($("select").get(4)).find("option").remove();
									var option = $(this);
									var lv4Data = $.grep(setMenus, function(o){
										return o.lv == '4' && o.pmenuId == option.val();
									});
									var leftIDs = [];
									$($("select").get(0)).find("option").each(function(){
										leftIDs.push($(this).val().replace(/(\d+\-)/g, ""));
									});
									$(lv4Data).each(function(){
										if($.inArray(this.menuId, leftIDs) < 0){
											$($("select").get(4)).append("<option lv='"+ this.lv +"' isLeaf='"+ this.isLeaf +"' value='"+ this.menuId +"'>"+ this.menuName +"</option>")
										}
									});	
