			...
			for(var i = 0; i < opts.customOperationConfig.length; i++){
				...
				$("#" + o.bindSubmitElement).click([o],function(callback){				
					...
					if(checkboxs){
							...
							if(callback.data[0].type == "single"){
...
//			/["+ currentPageBtn +"页]
				...
				if($(this).find("th").length > 0){			
					if(i == 0){
						if(opts.checkboxConfig.singleCheck){
							$(this).prepend("<th rowspan='"+ rowspan +"' style='width:12px;'></th>");
						}else{
							$(this).prepend("<th rowspan='"+ rowspan +"' style='width:12px;'><input _name='_chk_" + i + "' type='checkbox'/></th>");
						}
					}
						...
						if($(this).attr("checked")){
							$(checkboxs).each(function(){
								if(!$(this).attr("disabled")){
									...
									if(opts.checkboxConfig.handler){
										opts.checkboxConfig.handler($(this).parent().nextAll(), true);				
									}
									$(this).parent("td").parent("tr").children().each(function(){
										$(this).css({background:"#B5DAE3"});
									});
							...
							$(checkboxs).each(function(){
								if(opts.checkboxConfig.handler){
									opts.checkboxConfig.handler($(this).parent().nextAll(), false);				
								}
								$(this).parent("td").parent("tr").children().each(function(){
									$(this).css({background:""});
								});	
		...
		if(opts.checkboxConfig.disableCheckbox){
			var trs = $(this).find("tbody>tr");
			var checkboxs = $(this).find("tr>td>input:checkbox");
			
			if(trs){
				$(trs).each(function(i){
					var disable = $(trs[i]).children("td").eq(opts.checkboxConfig.disableCheckbox).text();

					if(disable == 'true'){
						$(checkboxs).eq(i).attr("disabled",true);
					}else{
						$(checkboxs).eq(i).attr("disabled",false);
					}
			...
			if(opts.checkboxConfig.handler){
				$(checkboxs).each(function(){
					$(this).click(function(){
						...
						opts.checkboxConfig.handler($(this).parent().nextAll(), checked);
					});
				});
		}	
