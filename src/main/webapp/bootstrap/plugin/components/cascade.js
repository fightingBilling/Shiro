$.extend(Component, {
	...
	change_select : function(options){
		var config = {
			// 请求url
			baseUrl : options.baseUrl, 
			...
			// 子控件
			item : options.itemId ? $("#" + options.itemId) : null,
			
			// 子控件Option映射,{value: null, text: null}
			itemOptionMapping : options.itemOptionMapping ? options.itemOptionMapping : null,
		...
		config.parent.change(function(){
			...
			if(!value){
				config.item.find("option").remove();
				return;
			}
			...
			$.post(config.baseUrl, param, function(result){

				$('body').modalmanager('loading');
				
				config.item.find("option").remove();
				
				config.item.append("<option value=''>请选择</option>");
				
				var data = $.evalJSON(result);
				$(data).each(function(){
					config.item.append("<option value='"+ this[config.itemOptionMapping.value] +"'>"+ this[config.itemOptionMapping.text] +"</option>");
				});
				
				$('body').modalmanager('loading');
				
				(config.callback)(data, config.item);
	...
	init_select: function(options){
		var config = {
			// 主控件
			item : options.itemId ? $("#" + options.itemId) : null,
			
			itemContext : options.itemContext ? $("#" + options.itemContext) : null,
		...
		var data =  $.evalJSON(config.itemContext.val());
	...
	cascade : function(options) {
		...
		var _options = {
			...
			childrenCallback : options.childrenCallback || function(){},
			// 是否异步级联
			...
			syncConfig : {
				...
				// 子控件Option映射,{value: null, text: null, ref:null},
				// ref-关联父控件的value
			...
			asyncConfig : {
				...
				childOptionAll : options.asyncConfig ? options.asyncConfig.childAll : null,
				// 请求参数名称
				baseParamName : {
					type: "_type",// 类型
					pid: "_pid"// 父ID
		...
		if($.type(options.childId) != "array"){
			_options.child = [$("#" + options.childId)];
		}else{
			_options.child = [];
			$(options.childId).each(function(){
				_options.child.push($("#" + this.id));
			});
		}
				...
				$.post(config.childBaseUrl, $.parseJSON('{"'+ config.baseParamName.type +'":"child", "'+ config.baseParamName.pid +'":"'+ seletedId +'"}'), function(result){
					...
					c.each(function(){
						...
						if(data[0]){
							...
							if(config.childOptionAll == 'all'){
	
								c.append("<option value='all'>全部</option>");
							...
							$(data).each(function(){
								c.append("<option value='"+ this[config.childOptionMapping.value] +"'>"+ this[config.childOptionMapping.text] +"</option>");
							});
						}
					});

					$('body').modalmanager('loading');
					// console.log("invoke childrenCallback")
					_options.childrenCallback(data);
				}, "text");
			}else{
				
				$(_options.child).each(function(){
