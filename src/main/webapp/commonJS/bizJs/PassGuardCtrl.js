		...
		prototype: {
			...
			getpgeHtml: function() {
				if (this.osBrowser==1 || this.osBrowser==3) {
					...
					if(navigator.userAgent.toLocaleLowerCase().indexOf("qqbrowser")>0){//判断QQ浏览器
						var pgeOcx='<embed onmousemove="return this.focus()" ID="' + this.settings.pgeId + '"  maxlength="'+this.settings.pgeMaxlength+'" input_2="'+this.settings.pgeEreg1+'" input_3="'+this.settings.pgeEreg2+'" edittype="'+this.settings.pgeEdittype+'" type="application/pass-guard-x" tabindex="'+this.settings.pgeTabindex+'" class="' + this.settings.pgeClass + '" ';
			...
			load: function() {				
				if (!this.checkInstall()) {
					...
					if(this.osBrowser==1||this.osBrowser==3){
						...
						if(this.pgeVersion!=PGEdit_FF_VERSION && PGEdit_Update==1){//已安装控件版本号与最新版本号比较
							this.setDownText();
							return this.getDownHtml();
						} 
					}else if(this.osBrowser==2){  
